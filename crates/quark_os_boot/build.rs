...
fn main() {
  ...
  let entry_path = std::env::var("CARGO_BIN_FILE_QUARK_OS_ENTRY").unwrap();
  eprintln!("{entry_path:?}");
  let disk_builder = DiskImageBuilder::new(PathBuf::from(entry_path));

  let out_dir = PathBuf::from(std::env::var("OUT_DIR").unwrap());
  let uefi_path = out_dir.join("quark_os-uefi.img");
  let bios_path = out_dir.join("quark_os-bios.img");
